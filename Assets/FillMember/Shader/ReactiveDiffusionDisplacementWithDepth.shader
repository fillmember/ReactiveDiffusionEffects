Shader "Hidden/FillMember/ReactiveDiffusionDisplacementWithDepth"
{
	Properties
	{
	...
	float4 frag_rd(v2f source) : SV_Target {
		...
		// Simulation
		float depth = tex2D( _CameraDepthTexture , source.uv ).r;
		float ts = lerp( 0.0001 , texelSize , depth );
		float2 stepX = float2( ts , 0 );
		float2 stepY = float2( 0 , ts );
